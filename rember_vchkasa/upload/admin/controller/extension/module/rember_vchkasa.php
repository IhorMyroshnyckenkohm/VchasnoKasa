...
class ControllerExtensionModuleRemberVchkasa extends Controller
{
    ...
    public function install()
    {
        ...
        $this->db->query(
            ...
            VALUES ("' . $this->db->escape($this->language->get('example_device')) . '")'
        );
    }
...
//        don`t delete because when we install again we can create duplicate orders receipt
    ...
    public function btn_create_receipt(&$route, &$data, &$output)
    {
        ...
        if ($route === 'sale/order_info') {
            ...
            </a> " . $this->select_device(empty($this->session->data['device_id']));
        } else {
            $button .= "
        <button 
            id='button-receipt'  
            ...
            <i class='bi bi-receipt'></i>
        </button>" . $this->select_device(empty($this->session->data['device_id']));
    ...
    public function receipt()
    {
        ...
        if (empty($orders)) {
            ...
            foreach ((array)$orders[0] as $order_id) {
                ...
                $data['orders'][$order_id]['href'] = 'index.php?route=sale/order/info&user_token=' . $this->session->data['user_token'] . '&order_id=' . $order_id;
                ...
                if (key_exists('payment_status', $order_info) && $order_info['payment_status'] !== 'P') {
                    $data['orders'][$order_id]['response'] = $this->language->get('status_must_be_paid');
                    continue;
                }
...
//                            "comment" => "Payment comment for card"
                    ...
                    if (isset($response['info']['printinfo']['fisn'])) {
                        ...
                        $data['orders'][$order_id]['response'] = $this->language->get('success') . ' ' . $response['info']['printinfo']['fisn'];
    ...
    public function select_device($isEmpty)
    {
        if ($isEmpty) {
            $this->load();
            $html = "
    ...
    if (buttonReceipt) {
        ...
        if(buttonReceipt.classList.contains('select_device')) {
            ...
            if (saveDevice) {
                ...
                if (deviceSelect) {
                $.ajax({
		url: 'index.php?route=extension/module/rember_vchkasa/device_save_ajax&user_token=" . $this->session->data['user_token'] . "',
		type: 'post',
		dataType: 'json',
